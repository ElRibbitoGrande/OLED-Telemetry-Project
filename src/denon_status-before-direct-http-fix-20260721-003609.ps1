...
$script:httpClient.DefaultRequestHeaders.CacheControl = [System.Net.Http.Headers.CacheControlHeaderValue]::new()
$script:httpClient.DefaultRequestHeaders.CacheControl.NoCache = $true
$script:httpClient.DefaultRequestHeaders.CacheControl.NoStore = $true
...
function Start-XmlRequest($url) {
    try {
        # Add the cache-buster here so every XML caller uses one consistent path.
