...
function Apply-AudysseyXml($audyssey) {
    ...
    if ($null -ne $dynamicNode) { $script:state.DynamicEQ = Map-OnOff $dynamicNode.InnerText }
    if ($null -ne $rloNode) { $script:state.ReferenceLevelOffset = "{0} dB" -f $rloNode.InnerText }
...
function Get-FreshFastStatusXml {
    try {
        $cacheBust = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
        $url = "https://$ip`:10443/ajax/general/get_config?type=12&_=$cacheBust"
        $curl = "$env:SystemRoot\System32\curl.exe"
        $fastTemp = "C:\RainmeterDenon\denon_fast_status.tmp"

        # Write curl's response to a file rather than capturing native stdout.
        # Hidden Windows PowerShell sessions can intermittently lose native stdout,
        # even though the identical interactive curl command works correctly.
        & $curl -k -s --max-time 2 -o $fastTemp $url 2>$null
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $fastTemp)) { return $null }

        $raw = Get-Content -LiteralPath $fastTemp -Raw -ErrorAction Stop
        Remove-Item -LiteralPath $fastTemp -Force -ErrorAction SilentlyContinue
        if ([string]::IsNullOrWhiteSpace($raw)) { return $null }

        $soundModeRaw = ''
        $inputSignalRaw = ''
        if ($raw -match '<SoundMode>([^<]*)</SoundMode>') {
            $soundModeRaw = [System.Net.WebUtility]::HtmlDecode($Matches[1])
        }
        if ($raw -match '<InputSignal>([^<]*)</InputSignal>') {
            $inputSignalRaw = [System.Net.WebUtility]::HtmlDecode($Matches[1])
        }

        $xml = ConvertTo-DenonXml $raw
        if ($null -eq $xml) { return $null }

        # Small diagnostic file: confirms what the hidden collector actually read.
        $diagnostic = @"
SoundModeRaw=$soundModeRaw
InputSignalRaw=$inputSignalRaw
Updated=$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff')
"@
        Set-Content -LiteralPath "C:\RainmeterDenon\denon_fast_status.txt" -Value $diagnostic -Encoding ASCII

        return [pscustomobject]@{ Xml=$xml; SoundMode=$soundModeRaw; InputSignal=$inputSignalRaw }
    }
    catch {
        Set-Content -LiteralPath "C:\RainmeterDenon\denon_fast_status_error.txt" -Value ("{0}  {1}" -f (Get-Date -Format 'yyyy-MM-dd HH:mm:ss.fff'), $_.Exception.Message) -Encoding ASCII
        return $null
    }
}
...
function Start-XmlRequest($url) {
    try {
        # Denon's web interface can return a cached type=12 response when the URL
        # is identical. A unique query value plus no-cache headers forces a live
        # decoder-status snapshot, which is essential when switching between
        # PCM, DD+ Atmos, TrueHD Atmos, and other formats.
        $separator = if ($url.Contains("?")) { "&" } else { "?" }
        $cacheBust = [DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds()
        return $script:httpClient.GetStringAsync("$url$separator`_=$cacheBust")
    }
    catch { return $null }
}

function Complete-XmlTask($task, $applyScriptBlock) {
    if ($null -eq $task -or -not $task.IsCompleted) { return $false }
    if (-not $task.IsCanceled -and -not $task.IsFaulted) {
        try {
            $xml = ConvertTo-DenonXml $task.Result
            if ($null -ne $xml) { & $applyScriptBlock $xml }
        } catch {}
    }
    return $true
}
...
DynamicEQ=$($script:state.DynamicEQ)
...
try {
    while ($true) {
        ...
        # Start slow XML reads asynchronously. Their network latency no longer
        # blocks volume reads or Rainmeter status-file updates.
        if ($now -ge $nextFastXml) {
            $freshFastStatus = Get-FreshFastStatusXml
            if ($null -ne $freshFastStatus) {
                Apply-FastXml $freshFastStatus.Xml
                if (-not [string]::IsNullOrWhiteSpace($freshFastStatus.SoundMode)) {
                    $script:state.Mode = Format-ListeningMode $freshFastStatus.SoundMode
                }
                if (-not [string]::IsNullOrWhiteSpace($freshFastStatus.InputSignal)) {
                    $script:state.InputSignal = Format-InputSignal $freshFastStatus.InputSignal
                }
                Update-VisualMode
            }
            $nextFastXml = (Get-Date).AddMilliseconds($fastXmlIntervalMs)
        }
        if ($null -eq $audysseyXmlTask -and $now -ge $nextAudysseyXml) {
        ...
        if ($null -eq $presetXmlTask -and $now -ge $nextPresetXml) {
        ...
        if (Complete-XmlTask $audysseyXmlTask ${function:Apply-AudysseyXml}) { $audysseyXmlTask = $null }
        if (Complete-XmlTask $presetXmlTask ${function:Apply-PresetXml}) { $presetXmlTask = $null }
