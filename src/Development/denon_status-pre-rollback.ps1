...
$quickSelectNamesIntervalMs = 300000
...
# Use asynchronous HTTP for infrequent preset/name requests. Audyssey is
# polled directly with the verified curl command every five seconds.
...
$quickSelectNames = @{}
$httpsMutex = [Threading.Mutex]::new($false, "Local\RainmeterDenonHttps")
...
function Get-DenonXmlSynchronized([string]$url, [int]$maxTimeSeconds = 12) {
    $lockTaken = $false
    try {
        # All Denon HTTPS collectors share this mutex. The AVR web server behaves
        # like a single-lane endpoint; serializing requests prevents one slow
        # decoder query from starving Audyssey updates.
        # Never block the main telemetry loop behind a slow decoder request.
        # Volume and all TCP-fed telemetry must remain live. If HTTPS is busy,
        # skip this poll and retry on the next scheduled cycle.
        $lockTaken = $script:httpsMutex.WaitOne(0)
        if (-not $lockTaken) { return $null }

        $raw = & curl.exe -k -sS --connect-timeout 5 --max-time $maxTimeSeconds $url 2>$null
        if ($LASTEXITCODE -ne 0 -or $null -eq $raw) { return $null }

        $rawText = ($raw -join [Environment]::NewLine).Trim()
        if ([string]::IsNullOrWhiteSpace($rawText)) { return $null }

        return ConvertTo-DenonXml $rawText
    }
    catch {
        return $null
    }
        ...
        if ($lockTaken) {
            try { $script:httpsMutex.ReleaseMutex() } catch {}
        }
    }
}

function Get-AudysseyXml {
    $url = "https://$script:ip`:10443/ajax/audio/get_config?type=9"
    return Get-DenonXmlSynchronized $url 8
}

function Start-XmlRequest($url) {
    try {
        # The Audyssey (type=9) and speaker-preset (type=10) endpoints expect
        # their original query string. Appending an extra cache-buster parameter
        # can cause these Denon endpoints to return stale data or fail silently.
        # HttpClient already sends no-cache/no-store headers above.
        return $script:httpClient.GetStringAsync($url)
    }
    catch { return $null }
}
...
try {
    while ($true) {
        ...
        # Poll Audyssey directly. The verified request normally completes in a
        # fraction of a second; on failure, retain the last known good values.
        if ($now -ge $nextAudysseyXml) {
            $audysseyXml = Get-AudysseyXml
            if ($null -ne $audysseyXml) {
                Apply-AudysseyXml $audysseyXml
            }
            $nextAudysseyXml = (Get-Date).AddMilliseconds($audysseyIntervalMs)
        }

        # Keep every Denon HTTPS request on the same serialized path.
        if ($now -ge $nextPresetXml) {
            $presetXml = Get-DenonXmlSynchronized "https://$ip`:10443/ajax/globals/get_config?type=10" 8
            if ($null -ne $presetXml) {
                Apply-PresetXml $presetXml
            }
            $nextPresetXml = (Get-Date).AddMilliseconds($presetIntervalMs)
        }

        if ($now -ge $nextQuickSelectNamesXml) {
            $quickNamesXml = Get-DenonXmlSynchronized "https://$ip`:10443/ajax/globals/get_config?type=7" 12
            if ($null -ne $quickNamesXml) {
                Apply-QuickSelectNamesXml $quickNamesXml
                $nextQuickSelectNamesXml = (Get-Date).AddMilliseconds($quickSelectNamesIntervalMs)
            }
            else {
