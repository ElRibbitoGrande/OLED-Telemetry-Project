...
function Normalize-Format([string]$Value) {
    ...
    if ($v -match "ATMOS") { return "Dolby Atmos" }
    if ($v -match "DTS:X") { return "DTS:X" }
    if ($v -match "DTS[- ]HD.*(MSTR|MASTER|MA)") { return "DTS-HD MA" }
    if ($v -match "TRUEHD") { return "Dolby TrueHD" }
    if ($v -match "DOLBY AUDIO - DD\+|DOLBY DIGITAL PLUS|DD\+") { return "Dolby Digital Plus" }
    if ($v -match "DOLBY AUDIO - DD|DOLBY DIGITAL|^DD$") { return "Dolby Digital" }
    if ($v -match "MULTICHANNEL PCM|PCM MULTI|MULTI CH") { return "Multichannel PCM" }
    if ($v -match "^PCM$|2\.0.*PCM|PCM.*2\.0") { return "Stereo PCM" }
    if ($v -match "^DTS$") { return "DTS" }

    return (Get-Culture).TextInfo.ToTitleCase($v.ToLowerInvariant())
}
...
try {
    while ($true) {
        ...
        try {
            ...
            $httpsLockTaken = $false
            try {
                # Wait for the shared Denon HTTPS lane instead of skipping the
                # decoder update. The metadata router depends on current decoder
                # state to recognize the analog LYR+/WiiM path; leaving this file
                # stale can suppress the entire Now Playing block.
                $httpsLockTaken = $httpsMutex.WaitOne(20000)
                if (-not $httpsLockTaken) {
                    throw "Timed out waiting for the Denon HTTPS interface."
                }
                ...
                if ($httpsLockTaken) {
                    try { $httpsMutex.ReleaseMutex() } catch {}
                }
            }
