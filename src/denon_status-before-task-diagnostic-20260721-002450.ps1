...
function Complete-XmlTask($task, $applyScriptBlock) {
    ...
    if (-not $task.IsCanceled -and -not $task.IsFaulted) {
        try {
            $task.Result | Set-Content "C:\RainmeterDenon\Development\AudysseyRaw.xml"

            $xml = ConvertTo-DenonXml $task.Result

            if ($null -eq $xml) {
                "ConvertTo-DenonXml returned `$null" |
                    Set-Content "C:\RainmeterDenon\Development\AudysseyException.txt"
            }
...
"@ | Add-Content "C:\RainmeterDenon\Development\AudysseyException.txt"
        }
    }
    return $true
}
...
try {
    while ($true) {
        ...
        if ($null -eq $quickSelectNamesCurlProcess -and
            ...
            $audysseyXmlTask = Start-XmlRequest "https://$ip`:10443/ajax/audio/get_config?type=9&_=$([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())"
